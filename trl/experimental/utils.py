...
class DataCollatorForChatML:
    ...
    _debug_empty_warning_count: int = 0
    ...
    def __call__(self, examples: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        for example in examples:
            ...
            if formatted_prompt is None:
                enable_thinking = "qwen3" in str(getattr(self.tokenizer, "name_or_path", "")).lower()
            ...
            if "input_ids" not in example:
                ...
                if len(message_input_ids) == 0:
                    # Guard against zero-length sequences which can crash attention kernels.
                    fallback_id = (
                    ...
                    if fallback_id is None:
                        raise ValueError("Tokenizer must define eos_token_id or pad_token_id for empty sequences.")
                    if self._debug_empty_warning_count < 5:
                        warnings.warn(
                            f"Empty tokenized sequence detected (keys={list(example.keys())}); "
                            "inserting fallback token to avoid invalid GEMM.",
                            stacklevel=2,
                        )
                        self._debug_empty_warning_count += 1
                    message_input_ids = [fallback_id]
                ...
                if len(message_input_ids) == 0:
                    ...
                    if self._debug_empty_warning_count < 5:
                        warnings.warn(
                            f"Empty `input_ids` detected (keys={list(example.keys())}); "
                            "inserting fallback token to avoid invalid GEMM.",
                            stacklevel=2,
                        )
                        self._debug_empty_warning_count += 1
                ...
                if len(current_prompt_ids) == 0:
                    current_prompt_ids = message_input_ids[:]
            ...
            if len(current_prompt_ids) == 0:
                ...
                if self._debug_empty_warning_count < 5:
                    warnings.warn(
                        f"Empty prompt sequence detected (keys={list(example.keys())}); "
                        "inserting fallback token to avoid invalid GEMM.",
                        stacklevel=2,
                    )
                    self._debug_empty_warning_count += 1
                current_prompt_ids = [fallback_id]
