...
class GOLDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        if data_collator is None:
            data_collator = DataCollatorForChatML(
                ...
                prompt_key="original_prompt_text",
        ...
        self._warned_empty_eval_labels = False
    ...
    def _prepare_dataset_with_original_text(
        ...
        with PartialState().main_process_first():
            ...
            # Normalize dataset fields so prompt/completion datasets are read consistently.
            def _normalize_completion(example):
                if "completion" not in example and "generations" in example:
                    example["completion"] = example["generations"]
                return example

            dataset = dataset.map(_normalize_completion, **map_kwargs)
            ...
            def add_eos(example, eos_token):
                ...
                if "text" in example and not example["text"].endswith(eos_token):  # language modeling case
                    ...
                    completion_key = next((k for k in completion_keys if k in example), None)
                    if completion_key is not None:
                        completion_text = example[completion_key]
                        if isinstance(completion_text, str) and not completion_text.endswith(eos_token):
            ...
            def tokenize_with_original_text(example, processing_class, dataset_text_field, assistant_only_loss):
                ...
                if "prompt" in example:  # prompt-completion or prompt-only case
                    completion_text = example.get("completion")
                    if completion_text is None:
                        for key in completion_keys:
                            if key in example:
                                completion_text = example[key]
                                break
                    if completion_text is None and dataset_text_field in example:
                        completion_text = example[dataset_text_field]
                    # Store original text
                    result["original_prompt_text"] = example["prompt"]
                    result["original_completion_text"] = completion_text

                    if is_conversational(example):
                        enable_thinking = "qwen3" in str(getattr(processing_class, "name_or_path", "")).lower()
                        ...
                        if completion_text:
                            prompt_completion_ids = processing_class.apply_chat_template(
                                example["prompt"] + completion_text,
                                return_dict=False,
                                **({"enable_thinking": True} if enable_thinking else {}),
                                **example.get("chat_template_kwargs", {}),
                            )
                        ...
                        if completion_text:
                            prompt_completion_ids = processing_class(
                                text=example["prompt"] + completion_text
                            ).input_ids
                        else:
                            prompt_completion_ids = prompt_ids
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_uld_loss:
            ...
            # Guard against empty label masks during evaluation (would zero the ULD loss).
            if not model.training and (student_labels != -100).sum().item() == 0:
                if not self._warned_empty_eval_labels:
                    logger.warning(
                        "Eval batch has no non-masked labels; falling back to full-sequence labels to avoid zero loss."
                    )
                    self._warned_empty_eval_labels = True
                student_labels = inputs["input_ids"].clone()
                if hasattr(self.processing_class, "pad_token_id") and self.processing_class.pad_token_id is not None:
                    student_labels[student_labels == self.processing_class.pad_token_id] = -100
                if (teacher_labels_for_loss != -100).sum().item() == 0:
                    teacher_labels_for_loss = student_labels
                    teacher_input_ids_for_loss = inputs["input_ids"]
    ...
    def generate_on_policy_outputs(self, model, inputs, generation_config, pad_token_id=None):
        ...
        if self.use_transformers_paged:
            ...
            prompts_tensor = inputs["prompts"]
            prompt_mask = inputs.get("prompt_attention_mask", None)
            ...
            if prompt_mask is not None:
                ...
                if pad_token_id is not None:
                    ...
                    prompt_lengths = torch.full(
                        (prompts_tensor.size(0),),
                        prompts_tensor.size(1),
                        dtype=torch.long,
                        device=prompts_tensor.device,
                    )

            if (prompt_lengths == 0).any():
                if not hasattr(self, "_debug_empty_prompt_warns"):
                    self._debug_empty_prompt_warns = 0
                if self._debug_empty_prompt_warns < 5:
                    warnings.warn(
                        "Zero-length prompts detected in on-policy generation; "
                        "inserting fallback token to avoid invalid GEMM.",
                        stacklevel=2,
                    )
                    self._debug_empty_prompt_warns += 1
                ...
                if fallback_id is None:
                    raise ValueError("Tokenizer must define eos_token_id or pad_token_id for empty prompts.")

                prompts_tensor = prompts_tensor.clone()
                prompts_tensor[prompt_lengths == 0, -1] = fallback_id
                if prompt_mask is not None:
                    prompt_mask = prompt_mask.clone()
                    prompt_mask[prompt_lengths == 0, -1] = 1

            if not hasattr(self, "_debug_prompt_shape_warns"):
                self._debug_prompt_shape_warns = 0
            if self._debug_prompt_shape_warns < 5:
                min_len = int(prompt_lengths.min().item()) if prompt_lengths.numel() else 0
                max_len = int(prompt_lengths.max().item()) if prompt_lengths.numel() else 0
                warnings.warn(
                    f"On-policy prompt stats: shape={tuple(prompts_tensor.shape)}, "
                    f"min_len={min_len}, max_len={max_len}, "
                    f"pad_token_id={pad_token_id}, eos_token_id={self.processing_class.eos_token_id}",
                    stacklevel=2,
                )
                self._debug_prompt_shape_warns += 1

            vocab_size = getattr(model.config, "vocab_size", None)
            if vocab_size is not None:
                min_id = int(prompts_tensor.min().item()) if prompts_tensor.numel() else -1
                max_id = int(prompts_tensor.max().item()) if prompts_tensor.numel() else -1
                if min_id < 0 or max_id >= vocab_size:
                    warnings.warn(
                        f"Prompt token id out of range: min_id={min_id}, max_id={max_id}, "
                        f"vocab_size={vocab_size}. Replacing out-of-range with eos_token_id.",
                        stacklevel=2,
                    )
                    fallback_id = (
                        self.processing_class.eos_token_id
                    ...
                    if fallback_id is None:
                        raise ValueError("Tokenizer must define eos_token_id or pad_token_id for token range fixes.")
                    prompts_tensor = prompts_tensor.clamp(0, vocab_size - 1)
                    prompts_tensor[prompts_tensor >= vocab_size] = fallback_id
                    prompts_tensor[prompts_tensor < 0] = fallback_id

            if prompt_mask is not None:
                if prompt_mask.shape != prompts_tensor.shape:
                    warnings.warn(
                        f"Prompt attention_mask shape mismatch: "
                        f"mask={tuple(prompt_mask.shape)}, prompts={tuple(prompts_tensor.shape)}. "
                        "Dropping attention_mask for generation.",
                        stacklevel=2,
                    )
                    prompt_mask = None
                else:
                    prompt_mask = (prompt_mask > 0).to(prompts_tensor.device)
                    if prompt_mask.numel() and (prompt_mask.sum(dim=1) == 0).any():
                        warnings.warn(
                            "Prompt attention_mask has all-zero rows; this can trigger invalid GEMM.",
                            stacklevel=2,
                        )
                    if not hasattr(self, "_debug_mask_warns"):
                        self._debug_mask_warns = 0
                    if self._debug_mask_warns < 5:
                        min_mask = int(prompt_mask.min().item()) if prompt_mask.numel() else 0
                        max_mask = int(prompt_mask.max().item()) if prompt_mask.numel() else 0
                        warnings.warn(
                            f"Prompt mask stats: shape={tuple(prompt_mask.shape)}, min={min_mask}, max={max_mask}",
                            stacklevel=2,
                        )
                        self._debug_mask_warns += 1
            ...
            if hasattr(model, "gradient_checkpointing_disable"):
                if getattr(model, "is_gradient_checkpointing", False) or getattr(model, "gradient_checkpointing", False):
            ...
            try:
                ...
                if gc_disabled and hasattr(model, "gradient_checkpointing_enable"):
