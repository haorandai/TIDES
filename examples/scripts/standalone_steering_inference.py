...
def load_steering_vectors(
    ...
    Returns:
        steering_vectors: Dict mapping layer_idx to normalized steering vector (unit norm)
        original_norms: Dict mapping layer_idx to original norm before normalization
    ...
    for pkl_path in layer_files:
        ...
        # Normalize to unit norm for consistent injection scaling
        vector = vector / original_norm

        steering_vectors[layer_idx] = vector
        print(
            f"  Layer {layer_idx:2d}: shape={vector.shape}, "
            f"original_norm={original_norm:.3f}, normalized_norm={torch.norm(vector).item():.3f}"
        )
